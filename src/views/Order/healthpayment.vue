<template>
	<div id="healthpayment" >
		...
		<div class="health-pay-money"><span>支付金额</span><i v-if="care_num == 36" >￥<em ref='moneyNum'>198000</em></i><i v-else-if="care_num == 30">￥<em  ref='moneyNum'>4980</em></i><i v-else><em ref='moneyNum'>&nbsp;</em></i></div>
		...
		getcycleorder(){
			...
			this.$http({
				...
				if (res.data.status == 2) {
					...
					return this.showDialog(res.data.msg)
...
//				return this.showDialog('请求服务器错误')
		...
		posteworderupdae(e){
			...
			if($('.health-pay-main li.on').length==0){
				this.showDialog('请选择周期方案')
				return
			}else if (this.address== null) {
				this.showDialog('请添加选择收货地址')
				return
			}
...
//					window.location.href='http://test.anmeicare.com/dist/#/order/healthorderpay?order_id='+order_id
					window.location.href='http://test.anmeicare.com/wechat.php/order/getopenid/order_id/'+order_id
				}
			}).catch((err)=>{
				return this.showDialog('请求服务器错误')
			})

<template>
	<div id="healthorderpay" v-if="shanxian==1">
		...
		<div class="health-pay-item">
			<h2 v-if="items.care_num == 36"><i class="icon iconfont icon-jiankangguanli"></i>方案：调理方案</h2>
			<h2 v-else-if="items.care_num == 30"><i class="icon iconfont icon-jiankangguanli"></i>方案：巩固方案</h2>
		...
		<div class="health-pay-money"><span><i class="icon iconfont icon-zhifu"></i>支付金额</span><i>￥<em ref='moneyNum'>{{items.pay_money}}</em></i></div>
		<p class="hint">点击[确认支付]按钮，即代表您已同意<router-link :to="{path:'/agreement',query: {id:3}}">《服务协议》</router-link></p>
...
//			alert(this.order_id)
	...
	methods: {
			getordermsg(){
				this.$http({
					method: 'post',
					url:'http://www.rt4st.org/wechat.php/order/healthorderpay',   
							...
							WeixinJSBridge.invoke(
						       ...
						       function(res){// 使用以上方式判断前端返回,微信团队郑重提示：res.err_msg将在用户支付成功后返回    ok，但并不保证它绝对可靠。     
						           if(res.err_msg == "get_brand_wcpay_request:ok" ) {
						           		$.ajax({
						           			url:'http://www.rt4st.org/wechat.php/order/orderquery',
											type:'post',
											dataType:'json',
											data:{
											...
											success:function(res){
												if(res.status == 1){
													window.location.href='http://www.rt4st.org/dist/#/myorder'
												}else{
													return this.$parent.showDialog('支付失败')
													window.location.href='http://www.rt4st.org/dist/#/myorder'
												}
...
<style scoped>
	...
	#healthorderpay>p.hint {
	    font-size: .26rem;
	    color: #bbbbbc;
	    margin: .35rem  0;
	}
	#healthorderpay>p.hint a {
	    font-size: .26rem;
	    color: #47cce2;
	}
	#healthpayment .health-pay-money i.icon-zhifu{
		...
		float: left;
	    color: #FA88A0;
		margin: .23rem .16rem 0 .1rem;
	    min-width: .24rem;
	    display: inline-block;
	}
	#healthpayment .icon-jiankangguanli{
		font-size: .36rem;
		margin-right: .1rem;
		color: #B2B2B2;
	}

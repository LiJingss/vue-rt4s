<template>
	<div id="payment">
		...
		<p>点击[立即支付]按钮，即代表您已同意<router-link :to="{path:'/agreement',query: {id:3}}">《服务协议》</router-link></p>
...
//				获取微信配置信息
				$.ajax({
					...
					url:'http://www.rt4st.org/wechat.php/pay/handpay',
					type:'post',
					dataType:'json',
					success:function(res){
						if (res.status == 1) {
							...
							WeixinJSBridge.invoke(
						       ...
						       function(res){// 使用以上方式判断前端返回,微信团队郑重提示：res.err_msg将在用户支付成功后返回    ok，但并不保证它绝对可靠。     
						           if(res.err_msg == "get_brand_wcpay_request:ok" ) {
						           		$.ajax({
						           			url:'http://www.rt4st.org/wechat.php/pay/orderquery',
											type:'post',
											dataType:'json',
											data:{
											...
											success:function(res){
												if(res.status == 1){
													window.location.href='http://www.rt4st.org/dist/#/hand/handdiagnosis'
												}else{
							...
							alert('支付失败')
							window.location.href='http://www.rt4st.org/dist/#/diagno'
						}else if(res.status == 2){
							alert('您还没有未登录')
							window.location.href='http://www.rt4st.org/dist/#/'
						}
					},
					error:function(){
						alert('请求服务器错误')

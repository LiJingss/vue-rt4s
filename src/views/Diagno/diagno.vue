		...
		methods: {
			diagno(){//判断是否支付
				if(this.status == 3){
					...
					// window.location.href='http://test.anmeicare.com/dist/#/order/payment?health_id'+health_id
					// this.$router.replace({path:'order/payment',query: {health_id:health_id}});
					window.location.href='http://test.anmeicare.com/wechat.php/pay/getopenid'
			...
			finishhand() {
				if(this.is_finish == 0){
					return this.showDialog('请先完成初诊手诊')
				}else {

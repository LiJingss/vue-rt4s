<template>  
	<div id="information">
		...
		<ul class="item-content profile-box-item">
			...
			<li><p><span>商户编码</span><input type="text" v-if="agentcode !=''" readonly unselectable="on" disabled="disabled" placeholder="若无商户编码，请留空" ref="agentcode" :value="agentcode"/><input type="text" v-else placeholder="若无商户编码，请留空" ref="agentcode" /></p></li>
		</ul>
...
export default {
	...
	methods: {  
		...
		putinformation() {
			...
			if (realname.value == '') {
				realname.focus();
				return this.$parent.showDialog('请输入姓名');
			...
			if (sex.value == '') {
				sex.focus();
				return this.$parent.showDialog('请选择性别');
			}; 
			// 出生年月不能为空		
			if (birthday.value == '') {
				birthday.focus();
				return this.$parent.showDialog('请选择出生年月');
			}; 
			// 出生地区不能为空		
			if(addressCodes == ''){
				return this.$parent.showDialog("请选择出生地区");
			}
			...
			if (address.value !='') {
				...
				if (this.checkChineseOnly(address_a) == false) {
					return this.$parent.showDialog('请输入正确的详细地址');
				};
			};
			//验证电话
			if (linkphone.value !='') {
				if (this.checkLinkphone(linkphone.value)== false) {
					return this.$parent.showDialog('请输入正确格式的电话');
				};
			};
			//验证邮箱
			if (email.value !='') {
				if (this.checkEmail(email.value) == false) {
					return this.$parent.showDialog('请输入正确的电子邮箱地址');
				};
			};
		...
		wechatConfig() {
			...
			.catch((err)=>{
				return this.$parent.showDialog(err)
			})
...
<style>  
	...
	#information .profile-box-item li:first-child p span{
		/*height: 1.5rem;*/
		line-height: 1.5rem; 
	}
	#information .bgsize {

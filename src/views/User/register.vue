<template>
	<div id="register">
		...
		<button v-if="tel!=''&&code!=''" class="blue-btn" @click="signIn()">注册/登录</button>
		<button v-else class="blue-btn" style="background:#cacdd6;">注册/登录</button>
		<p>注册即代表您已同意<router-link :to="{path:'/agreement',query: {id:2}}">《用户协议》</router-link></p>
	</div>
</template>
<script>
	export default {
			...
			signIn() {
				...
				if(logintel.value != ''){
					if(this.checkPhone(logintel.value) == false){
						this.$parent.showDialog('请输入正确手机号码')
						logintel.focus();
						return;
					}
				}else {
...
//						this.$router.replace({path:'/healthindex'})
						window.location.href='http://www.rt4st.org/wechat.php/user/getopenid'

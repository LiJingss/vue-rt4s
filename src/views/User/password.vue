<template>
	<div id="password">
		...
		<ul class="item-content">
			<li>
				<p><label style="width:100%;"><em class="iconfont icon-mima"></em>
				<input :type="pwdhide ? 'password' : 'text'" placeholder="设置密码（8-16位、两种字符组合）" v-model="pwd" ref='pwd' style="width:100%;"></label></p>
				...
				if(pwd.value != ''){
					if(this.checkPassword(pwd.value) == false){
						if(pwd.value.length < 8 || pwd.value.lenght >16){
							...
							this.$parent.showDialog('密码至少由两种字符组成');
							pwd.focus();
							return;
						}
					}
